				...
				for (Genome child : this.children) {
					if (child != null) {
						...
						if (this.train.getPopulation().getRules().isValid(child)) {
							...
							if (child instanceof NEATGenome) {
								NEATGenome neatChild=(NEATGenome)child;
								neatChild.setPerformedOperationName(((NamedEvolutionaryOperator)opp).getPerformedOperationName());
								if (opp.parentsNeeded()==1) {
									NEATGenome[] neatParents=new NEATGenome[1];
									neatParents[0]=(NEATGenome)parents[0];
									neatChild.setParents(neatParents);
								}else {
									NEATGenome[] neatParents=new NEATGenome[2];
									neatParents[0]=(NEATGenome)parents[0];
									neatParents[1]=(NEATGenome)parents[1];
									neatChild.setParents(neatParents);
								}
							}
